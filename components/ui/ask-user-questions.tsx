...
const AskUserQuestions = forwardRef<HTMLDivElement, AskUserQuestionsProps>(
  function AskUserQuestions(
    ...
    // Set on pointerdown (capture) so the focus handler can tell a click-driven
    // focus from a keyboard one and skip the blue ring. See the onFocus comment.
    ...
    const rowsContent = (
      <div
        ...
        // Capture phase: runs before the row's mousedown/focus, so the flag is
        // already set by the time onFocus decides whether to draw the ring.
        onPointerDownCapture={() => {
          pointerFocusRef.current = true;
        }}
        // Any key press means the user is on the keyboard again, so clear the
        // pointer flag and let the ring show for subsequent focus moves.
        onKeyDownCapture={() => {
        ...
        onFocus={(e) => {
          ...
          // morphing blue ring, which must only appear for KEYBOARD focus.
          //
          // :focus-visible alone isn't enough here: the row's onMouseDown
          // calls preventDefault() then .focus(), and a programmatic focus
          // after a prevented mousedown still matches :focus-visible — so a
          // plain click drew the ring. pointerFocusRef, set in that same
          // handler, lets us distinguish the two.
          ...
          if (indexAttr != null) {
            ...
            pointerFocusRef.current = false;
            setFocusedIndex(
              !fromPointer && (e.target as HTMLElement).matches(":focus-visible")
                ? idx
                : null
            );
          }
              ...
              {isFreeText ? (
                ...
                <div
                  ...
                  className={cn(
                    ...
                    // No focus outline/ring: the border stays constant and only
                    // the fill changes, so an active field reads as filled
                    // rather than outlined.
                    otherText.length > 0
                      ? "bg-active"
                      : "hover:bg-hover focus-within:bg-card"
                  )}
                >
                    ...
                    {showBack && (
                      ...
                      >
                        ...
                        <Button
                          ...
                          // Explicit resting colour: the ghost variant only sets
                          // a hover colour, so without this the label inherits
                          // whatever is ambient and can vanish against the card.
                          className="pl-3 sm:pl-[6px] text-muted-foreground hover:text-foreground"
                        >
                          <ArrowLeftKey />
                          Back
                        </Button>
                    ...
                    {showSkip && (
                      ...
                      >
                        ...
                        <Button
                          ...
                          // Arrow is desktop-only; restore symmetric padding on
                          // mobile where it's hidden, tighten for the icon on ≥sm.
                          // Explicit resting colour, same reason as Back above.
                          className="pr-3 sm:pr-[6px] text-muted-foreground hover:text-foreground"
                        >
                          {skipLabel}
                          <ArrowRightKey />
                        </Button>
